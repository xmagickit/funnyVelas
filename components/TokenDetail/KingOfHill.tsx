...
const KingOfHill = ({ token, vlxPrice }: { token: coinInfo, vlxPrice: number }) => {
    ...
    useEffect(() => {
        const value = Math.floor((_token.price || 0) * 2_000_000_000 / (token.graduationMarketCap || 5) * 100)
        setProgress(value);
    }, [_token, setProgress]);
    ...
    useEffect(() => {
        const handleUpdateBondingCurve = (
            ...
            if (_token._id === data.tokenId) {
                const value = Math.floor(((data.price || 0) * 2_000_000_000) / (token.graduationMarketCap || 5) * 100)
                setProgress(value);
    ...
    return (
        <div className="rounded-xl sm:rounded-2xl p-4 sm:p-5 lg:mt-3 mb-4 border dark:border-gray-700 border-gray-200">
            <h4 className="text-sm md:text-base font-semibold !leading-none pb-4 md:pb-5">King of the hill progress : {Math.min(progress, 100)}%</h4>
            ...
            <div>
                {_token.kingDate ?
                    <p className="text-xs font-normal leading-normal flex gap-1">
                        ...
                        Crowned king of the hill on {moment(_token.kingDate).format('MM/DD/YYYY, hh:mm:ss A')}
                    </p>
                    :
                    <p className="text-xs font-normal leading-normal">
                        Dethrone the current king at ${(vlxPrice / 2 * (_token.graduationMarketCap || 5)).toLocaleString()} market cap
                    </p>
                }
            </div>
        </div>
