...
export interface TermsSettingProps {
    terms: string;
}

const TermsEditor = () => {
    const { adminData, setAdminData } = useData();
    const [content, setContent] = useState<string>(adminData?.terms || '')

    const { setValue, handleSubmit, reset } = useForm<TermsSettingProps>();

    const onSubmit: SubmitHandler<TermsSettingProps> = async (_data) => {
        try {
            const data = await updateAdmin(_data)
            setAdminData(data)
            successAlert('Update Terms Successfully')
        } catch {
            errorAlert('Failed to update terms')
        }
    }
                        ...
                        <form onSubmit={handleSubmit(onSubmit)}>
                            ...
                            <div className="flex justify-end gap-4.5">
                                ...
                                <button
                                    ...
                                    disabled={content === adminData?.terms}
                                    type="submit"
                                >
                                    Save
                                </button>
