...
import { updateBannerInfo } from "@/utils/api";
...
const BannerSetting = () => {
    ...
    const onSubmit: SubmitHandler<BannerSettingProps> = async (_data) => {
        try {
            const formData = new FormData();
            formData.append('bannerTitle', _data.bannerTitle);
            formData.append('bannerContent', _data.bannerContent);
            if (_data.bannerUrl) formData.append('bannerUrl', _data.bannerUrl);
            else if (previewImage === null) formData.append('bannerRemoved', 'yes');
            const data = await updateBannerInfo(formData);
            setAdminData(data)
            successAlert('Update Banner Info Successfully')
        } catch {
            errorAlert('Failed to update banner info')
        }
    }
                            ...
                            <div className="flex justify-end gap-4.5">
                                <button
                                    ...
                                    onClick={() => {
                                        ...
                                        setPreviewImage(adminData?.bannerUrl ? (BACKEND_URL + adminData?.bannerUrl) : null)
                                    }}
                                >
                                    Cancel
                                </button>
