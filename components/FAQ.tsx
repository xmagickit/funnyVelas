    ...
    return (
        <div className="p-4 sm:p-6 xl:p-9">
            <div className="grid grid-cols-1 gap-x-7.5 gap-y-7.5 sm:gap-y-12.5 xl:grid-cols-2 xl:gap-y-17.5">
                {faqs.map(faq => (
                    <div className="flex gap-6" key={faq._id}>
                        <div className="flex h-15 w-full max-w-15 items-center justify-center rounded-xl bg-primary text-white">
                            <svg
                                className="fill-current" width="33" height="33" viewBox="0 0 33 33" fill="none"
                                ...
                                <defs>
                                    <clipPath id="clip0_1871_19072">
                                        <rect width="32" height="32" fill="white" transform="translate(0.984375 0.913086)">
                                        </rect>
                                    </clipPath>
                                </defs>
                            </svg></div>
                        <div className="flex w-full flex-col gap-6">
                            <h4 className="text-title-sm2 font-bold text-black dark:text-white">{faq.question}</h4>
                            <p className="w-full max-w-[452px] font-medium">{faq.answer}</p>
                        </div>
                    </div>
                ))}
            </div>
